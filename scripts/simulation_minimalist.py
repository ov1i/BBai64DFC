#!/usr/bin/env python3
import socket, struct, threading, collections, math
import numpy as np
import cv2
...
# tuple indices after struct.unpack() (0-based)
FIELD = {
...
Pdiag_slice = slice(36, 51)  # if you want to read it


class FrameAssembler:
    """Chunked grayscale frame assembler for multiple slots (0,1,...)"""
    def __init__(self, chunk_bytes=CHUNK_BYTES):
        self.chunk_bytes = chunk_bytes
        self.cur = {}   # (slot, frame_id) -> {buf, got[], w,h,ts,chunks}
        self.last = {}  # slot -> (np.uint8 HxW, ts_ns)
...
class TelemetryState:
    def __init__(self): self.lock = threading.Lock(); self.t = None
    def update(self, tup): 
...
def draw_drone_3dish(img, cx, cy, yaw, pitch, roll, arm_len=72):
    cv2.ellipse(img,(cx+8,cy+12),(arm_len,int(arm_len*0.35)),0,0,360,(25,25,25),-1)
    c,s = math.cos(yaw), math.sin(yaw)
    t_pitch = 1.0 - 0.25*abs(pitch); t_roll = 1.0 - 0.25*abs(roll)
    w1 = int(9*t_pitch)+1; w2 = int(9*t_roll)+1
    ...
    cv2.line(img,(x1a,y1a),(x1b,y1b),(60,160,255),w1+3); cv2.line(img,(x1a,y1a),(x1b,y1b),(130,230,255),w1)
    cv2.line(img,(x2a,y2a),(x2b,y2b),(60,160,255),w2+3); cv2.line(img,(x2a,y2a),(x2b,y2b),(130,230,255),w2)
    for (px,py) in [(x1a,y1a),(x1b,y1b),(x2a,y2a),(x2b,y2b)]:
        cv2.circle(img,(px,py),13,(40,40,40),-1); cv2.circle(img,(px,py),13,(100,100,100),1)
        cv2.line(img,(px-12,py),(px+12,py),(180,255,255),2); cv2.line(img,(px,py-12),(px,py+12),(180,255,255),2)
    cv2.circle(img,(cx,cy),12,(240,240,0),-1); cv2.circle(img,(cx,cy),12,(0,0,0),1)
    nose = (int(cx + 18*s), int(cy + 18*c)); cv2.line(img,(cx,cy),nose,(0,255,0),3)
...
def main():
    ...
    while True:
        ...
        if t and of_valid:
            ...
            end = (int(start[0] + t[FIELD['of_u']]*1.0),
                   int(start[1] + t[FIELD['of_v']]*1.0))
